...
echo "================================"
echo ""
echo "This script automates the setup of recommended configurations from the Claude Code tips repository."
echo "It installs the dx plugin and configures essential settings for optimal Claude Code usage."
echo ""

echo -e "${YELLOW}INSTALLS:${NC}"
echo "  1. DX plugin - installs skills like /dx:gha, /dx:handoff, /dx:half-clone, and more"
echo ""

echo -e "${YELLOW}SETTINGS (~/.claude/settings.json):${NC}"
echo "  2. Status line - custom status line showing model, git branch, uncommitted files, and token usage"
echo "  3. Disable auto-updates - prevents Claude Code from auto-updating (saves tokens)"
echo "  4. Lazy-load MCP tools - only loads MCP tool definitions when needed, saves context"
echo "  5. Read(~/.claude) permission - allows half-clone to read conversation history"
