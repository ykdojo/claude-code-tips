...
const EXPECTED_HASHES = {
  npm: '9a68304028647967dda465ad13d8a3533f20564a312e710f8183cf72ba1d8df1', // Placeholder
  'native-linux-x64': '9a68304028647967dda465ad13d8a3533f20564a312e710f8183cf72ba1d8df1',
...
const patches = [
  ...
  { name: 'Slim Bash.description param (~300 → 40 chars)', file: 'bash-description-param' },
  {
    ...
    customRegex: /if\(([a-zA-Z0-9_$]+)\.additionalContexts&&([a-zA-Z0-9_$]+)\.additionalContexts\.length>0\)Z\.messages\.push\(kK\(\{type:"hook_additional_context",content:\1\.additionalContexts,hookName:"UserPromptSubmit",toolUseID:`hook-\$\{([a-zA-Z0-9_$]+)\.randomUUID\(\)\}`,hookEvent:"UserPromptSubmit"\}\)\);/,
    customReplace: 'if($1.additionalContexts&&$1.additionalContexts.length>0&&!Z.messages.some(m=>JSON.stringify(m).includes(JSON.stringify($1.additionalContexts[0]))))Z.messages.push(kK({type:"hook_additional_context",content:$1.additionalContexts,hookName:"UserPromptSubmit",toolUseID:`hook-${$3}.randomUUID()`,hookEvent:"UserPromptSubmit"}));',
  },
  {
  ...
  },
  {
    ...
    customRegex: /if\(([a-zA-Z0-9_$]+)\.additionalContexts\?\.length\)K\.push\(kK\(\{type:"hook_additional_context",content:\1\.additionalContexts,hookName:"UserPromptExpansion",toolUseID:`hook-\$\{([a-zA-Z0-9_$]+)\.randomUUID\(\)\}`,hookEvent:"UserPromptExpansion"\}\)\);/,
    customReplace: 'if($1.additionalContexts?.length&&!K.some(m=>JSON.stringify(m).includes(JSON.stringify($1.additionalContexts[0]))))K.push(kK({type:"hook_additional_context",content:$1.additionalContexts,hookName:"UserPromptExpansion",toolUseID:`hook-${$3}.randomUUID()`,hookEvent:"UserPromptExpansion"}));',
  },
  {
