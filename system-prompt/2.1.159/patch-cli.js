...
// Configuration
const EXPECTED_VERSION = '2.1.159';
const EXPECTED_HASHES = {
  npm: 'c3a96240856fa15c6f821dd30227fd08252f6f7513502643f52f0319a8e6674e', // Placeholder
  'native-linux-x64': 'c3a96240856fa15c6f821dd30227fd08252f6f7513502643f52f0319a8e6674e',
  'native-linux-arm64': 'TODO: Update hash',
  'native-macos-arm64': 'TODO: Update hash',
};
...
const patches = [
  ...
  {
    name: 'Dedup UserPromptSubmit hook context',
    customRegex: /if\(([a-zA-Z0-9_$]+)\.additionalContexts&&([a-zA-Z0-9_$]+)\.additionalContexts\.length>0\)Z\.messages\.push\(([a-zA-Z0-9_$]+)\(\{type:"hook_additional_context",content:\1\.additionalContexts,hookName:"UserPromptSubmit",toolUseID:`hook-\$\{([a-zA-Z0-9_$]+)\.randomUUID\(\)\}`,hookEvent:"UserPromptSubmit"\}\)\);/,
    customReplace: 'if($1.additionalContexts&&$1.additionalContexts.length>0&&!Z.messages.some(m=>JSON.stringify(m).includes(JSON.stringify($1.additionalContexts[0]))))Z.messages.push($3({type:"hook_additional_context",content:$1.additionalContexts,hookName:"UserPromptSubmit",toolUseID:`hook-${$4}.randomUUID()`,hookEvent:"UserPromptSubmit"}));',
  },
  {
    name: 'Dedup UserPromptSubmit hook success',
    customRegex: /if\(([a-zA-Z0-9_$]+)\.message\)switch\(\1\.message\.attachment\.type\)\{case"hook_success":if\(!\1\.message\.attachment\.content\)break;Z\.messages\.push\(\1\.message\);break;default:Z\.messages\.push\(\1\.message\);break\}/,
    customReplace: 'if($1.message){if(!Z.messages.some(m=>JSON.stringify(m).includes(JSON.stringify($1.message.attachment?.content))))switch($1.message.attachment.type){case"hook_success":if(!$1.message.attachment.content)break;Z.messages.push($1.message);break;default:Z.messages.push($1.message);break}}',
  },
  {
    name: 'Dedup SessionStart hook success',
    customRegex: /if\(([a-zA-Z0-9_$]+)\.message\)A\.push\(\1\.message\);/,
    customReplace: 'if($1.message&&!A.some(m=>JSON.stringify(m).includes(JSON.stringify($1.message.attachment?.content))))A.push($1.message);',
  },
  {
    name: 'Dedup SessionStart hook context',
    customRegex: /if\(([a-zA-Z0-9_$]+)\.additionalContexts&&([a-zA-Z0-9_$]+)\.additionalContexts\.length>0\)Y\.push\(\.\.\.\1\.additionalContexts\);/,
    customReplace: 'if($1.additionalContexts)for(let c of $1.additionalContexts)if(!Y.includes(c))Y.push(c);',
  },
  {
    name: 'Dedup UserPromptExpansion hook context',
    customRegex: /if\(([a-zA-Z0-9_$]+)\.additionalContexts\?\.length\)K\.push\(([a-zA-Z0-9_$]+)\(\{type:"hook_additional_context",content:\1\.additionalContexts,hookName:"UserPromptExpansion",toolUseID:`hook-\$\{([a-zA-Z0-9_$]+)\.randomUUID\(\)\}`,hookEvent:"UserPromptExpansion"\}\)\);/,
    customReplace: 'if($1.additionalContexts?.length&&!K.some(m=>JSON.stringify(m).includes(JSON.stringify($1.additionalContexts[0]))))K.push($2({type:"hook_additional_context",content:$1.additionalContexts,hookName:"UserPromptExpansion",toolUseID:`hook-${$3}.randomUUID()`,hookEvent:"UserPromptExpansion"}));',
  },
  {
    name: 'Dedup UserPromptExpansion hook success',
    customRegex: /if\(([a-zA-Z0-9_$]+)\.message&&!\(\1\.message\.type==="attachment"&&\1\.message\.attachment\.type==="hook_success"&&\1\.message\.attachment\.content===""\)\)K\.push\(\1\.message\)/,
    customReplace: 'if($1.message&&!($1.message.type==="attachment"&&$1.message.attachment.type==="hook_success"&&$1.message.attachment.content==="")&&!K.some(m=>m.attachment?.content===$1.message.attachment?.content))K.push($1.message)',
  },
